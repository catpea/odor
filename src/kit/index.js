...
// Collects all packets until the expected count is reached, then sends
// one aggregate as { ...lastPacket, _collected: [...], _total: N }.
// Each packet must carry the expected count (default key: '_totalPosts').
export function accumulate(countKey = '_totalPosts') {
  const items = [];
  return (send, packet) => {
    items.push(packet);
    const total = packet[countKey];
    if (total !== undefined && items.length >= total) {
      send({ ...packet, _collected: [...items], _total: items.length });
    }
  };
}
