...
export default function skipUnchanged(manifest) {
  ...
  return async (send, packet) => {
    ...
    if (!entry || !outputExists) {
      ...
      manifestUpdates.set(postId, { fingerprint });
      send({ ...packet, _inputFingerprint: fingerprint });
      return;
    }
    ...
    if (allMtimeMatch) {
      ...
      send({ ...packet, _cached: true, _cachedResults: entry.results });
      return;
    }
    ...
    if (compositeHash === entry.compositeHash) {
      ...
      manifestUpdates.set(postId, { fingerprint, results: entry.results });
